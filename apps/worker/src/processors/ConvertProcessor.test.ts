...
import { ConvertProcessor } from './ConvertProcessor.js';

const { mockGetObject, mockPutObject, mockSharp, mockJpeg, mockPng, mockWebp } = vi.hoisted(() => {
  const mockToBuffer = vi.fn().mockResolvedValue(Buffer.from('converted-output'));
  return {
    mockGetObject: vi.fn(),
    mockPutObject: vi.fn(),
...
const mockInputFile = {
  ...
  mimeType: 'image/png',
  size: 1000,
  storagePath: 'uploads/user-1/photo.png',
...
function makeDeps(overrides?: { findById?: unknown; type?: string }) {
...
describe('ConvertProcessor', () => {
  ...
  it('downloads input, converts with sharp, uploads output, creates file record, completes job', async () => {
    const { jobRepo, fileRepo, redis, bullJob, findById, create, updateStatus, publish } = makeDeps(
      { type: 'jpeg' }
    );
    await new ConvertProcessor().process(bullJob, jobRepo, fileRepo, redis);

    expect(findById).toHaveBeenCalledWith('user-1', 'file-1');
    expect(mockGetObject).toHaveBeenCalledWith('test-bucket', 'uploads/user-1/photo.png');
  ...
  it('uses png() for format "png"', async () => {
    const { jobRepo, fileRepo, redis, bullJob } = makeDeps({ type: 'png' });
    await new ConvertProcessor().process(bullJob, jobRepo, fileRepo, redis);
    expect(mockPng).toHaveBeenCalled();
  });

  it('uses webp() for format "webp"', async () => {
    const { jobRepo, fileRepo, redis, bullJob } = makeDeps({ type: 'webp' });
    await new ConvertProcessor().process(bullJob, jobRepo, fileRepo, redis);
  ...
  it('throws if input file is not found', async () => {
    ...
    await expect(new ConvertProcessor().process(bullJob, jobRepo, fileRepo, redis)).rejects.toThrow(
      'Input file not found: file-1'
    );
  });

  it('publishes progress events at 0%, 33%, and 66% then job:completed', async () => {
    const { jobRepo, fileRepo, redis, bullJob, publish } = makeDeps({ type: 'jpeg' });
    await new ConvertProcessor().process(bullJob, jobRepo, fileRepo, redis);
