...
- Zustand v5 (client state: auth store, WebSocket store)
...
- Fastify v5 (HTTP framework with JSON schema validation)
...
- **No direct `db` access in route handlers** — all database access must go through the repository layer
- **No raw SQL strings outside `packages/db`** — Drizzle query builder is the default; `db.execute(sql\`...\`)` is only allowed inside repository methods in `packages/db/src/`
- **No `localStorage` or `sessionStorage` for tokens** — access tokens live in the Zustand memory store only
- **No `useEffect` + `fetch` for server data** — use TanStack Query (`useQuery`, `useMutation`) for all server state
- **No `.then()` chains** — use `async/await` everywhere except inside Axios interceptors where `.then()` is required by the interceptor contract
- **No `any` type** — use `unknown` and narrow with type guards; `any` is a lint error
- **No `console.log` in production code** — use the Pino logger instance (`request.log`, `server.log`, or the injected logger)
- **No non-null assertions (`!`)** without an inline comment explaining why the value is guaranteed to be non-null
...
| Classes (services, repositories) | `PascalCase` file + named export | `FileService.ts` → `export class FileService` |
| Hooks | `camelCase` prefixed with `use` | `useJobWebSocket.ts` |
| Utility functions | `camelCase` | `formatBytes.ts` |
| Config / non-class files | `camelCase` or `kebab-case` | `config.ts`, `drizzle.config.ts` |
| Directories | `lowercase` or `kebab-case`, plural for groupings | `routes/`, `services/`, `query-keys/` |
| Test files | Co-located, same name + `.test.ts` / `.test.tsx` | `FileService.test.ts` |
| Exports | Named exports everywhere | Exception: React page-level route components use default exports |
...
1. **Define types** in `packages/types/src/` — add domain types, enums, and API request/response shapes
...
4. **Add repository method(s)** in `apps/api/src/repositories/` — one method per data access operation
5. **Add service method(s)** in `apps/api/src/services/` — orchestrate repository calls, throw typed domain errors
6. **Add route handler(s)** in `apps/api/src/routes/` — define Fastify JSON schema, call service, return response
7. **Add query key(s)** in `apps/web/src/utils/queryKeys.ts` — follow the existing factory pattern
8. **Add API client function(s)** in `apps/web/src/api/` — typed Axios wrappers that call the new route(s)
9. **Build UI** in `apps/web/src/features/<feature>/` — components, hooks, and pages using TanStack Query
10. **Write unit tests** for service and repository methods co-located as `*.test.ts` next to the file under test
